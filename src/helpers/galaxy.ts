...
const getPlanetType = ({
  ...
  if (energy < 0.5 && acousticness > 0.5 && danceability >= 0.5) {
    /** Rocky Planet: Lower energy and higher acousticness for a "grounded" feeling,
     * With medium to high danceability representing the solid surface and movement */
    return 'rocky';
  }
  if (energy > 0.7 && valence > 0.7 && danceability >= 0.5) {
  ...
  if (energy <= 0.6 && acousticness > 0.6 && danceability < 0.6) {
  ...
  if (energy < 0.5 && valence > 0.7 && danceability < 0.5) {
  ...
  if (energy > 0.7 && valence < 0.3 && danceability >= 0.5) {
...
export const getCelestialBodyType = (audioFeatures: AudioFeatures): string => {
   ...
   */
  if (audioFeatures.energy < 0.3 && audioFeatures.valence < 0.3) {
    return 'black-hole';
  }
  /**
   ...
   */
  if (audioFeatures.energy > 0.8 && audioFeatures.acousticness < 0.3) {
    return 'supernova';
  }
